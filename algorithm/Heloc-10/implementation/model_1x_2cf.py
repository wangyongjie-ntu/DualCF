...
class MyDataset(Dataset):
    ...
    def __init__(self, query_x, cf_x, ccf_x, query_y, cf_y, ccf_y):
        super()
    ...
    def __getitem__(self, index):
        ...
        train_x = np.concatenate((query_x, cf_x, ccf_x), axis = 0)
        train_y = np.concatenate((query_y, cf_y, ccf_y), axis = 0)
...
if __name__ == "__main__":
    ...
    dquery = np.load("../../../data/Heloc-10/implementation/heloc_query.npy").astype(np.float32)
    dtest = np.load("../../../data/Heloc-10/implementation/heloc_test.npy").astype(np.float32)
    dcf = np.load("../../../data/Heloc-10/implementation/heloc_query_cf.npy").astype(np.float32)
    dccf = np.load("../../../data/Heloc-10/implementation/heloc_query_2cf.npy").astype(np.float32)

    query_x, query_y = dquery[:, 0:10], dquery[:, 10:11]
    cf_x, cf_y = dcf[:, 0:10], dcf[:, 10:11]
    ccf_x, ccf_y = dccf[:, 0:10], dccf[:, 10:11]
    test_x, test_y = dtest[:, 0:10], dtest[:, 10:11]
    ...
    for seed in range(100):
        ...
        Query = MyDataset(query_sx, cf_sx, ccf_sx, query_sy, cf_sy, ccf_sy)
