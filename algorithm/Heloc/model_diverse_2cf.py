...
if __name__ == "__main__":
    ...
    dcf = np.load("../../data/boston_housing/boston-plaincf-l2.npy").astype(np.float32)
    dccf = np.load("../../data/boston_housing/boston-plain2cf-l2.npy").astype(np.float32)
    ...
    E = utils.get_sympoly(D, length, flag_gpu=False)

    acc_list = []
    for seed in range(100):
        print("seed used :{}".format(seed))
        #random.seed(seed)
        idx  = dpp.sample(D, V, E=E, k=length, flag_gpu=False)
