...
def train(model, iterator, optimizer, criterion, device):
    ...
    for batch_idx, (data, target) in enumerate(iterator):
        ...
        data, target = data.reshape(-1, 23), target.reshape(-1, 1)
        optimizer.zero_grad()
        output = model(data)
...
if __name__ == "__main__":
    ...
    dcf = np.load("../../data/Heloc/heloc-plaincf-l2.npy").astype(np.float32)
    dccf = np.load("../../data/Heloc/heloc-plainccf-l2.npy").astype(np.float32)

    query_x, query_y = dquery[:, 0:23], dquery[:, 23:24]
    cf_x, cf_y = dcf[:, 0:23], dcf[:, 23:24]
    ccf_x, ccf_y = dccf[:, 0:23], dccf[:, 23:24]
