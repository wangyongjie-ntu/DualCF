...
class TripletDataset(data.Dataset):
    ...
    def __init__(self, dquery_x, dquery_y, dcf_x, dcf_y, perturb_method = None, perturb_num = 10):
        super().__init__()
...
class CFJointLoss(nn.Module):
    ...
    def forward(self, query_x, cf_x, base_x, pred_x, pred_cf, pred_base, target_x, target_cf):
        ...
        print(classification_loss, triplet_loss)
...
def train(model, iterator, optimizer, criterion, device):
    ...
    for batch_idx, (query_x, cf_x, base_x, query_y, cf_y) in enumerate(iterator):
        ...
        loss = criterion(query_x, cf_x, base_x, output1, output2, output3, query_y, cf_y)
        loss.backward()
        optimizer.step()
    ...
    return epoch_loss / (len(iterator.dataset) * 10), acc
...
if __name__ == "__main__":
    ...
    for seed in range(1):
        ...
        Train = TripletDataset(query_sx, query_sy, cf_sx, cf_sy)
        ...
        for epoch in range(500):
            ...
            train_loss, train_acc = train(model, train_loader, optimizer, criterion2, device)
            valid_loss, valid_acc = evaluate(model, val_loader, criterion1, device)
        ...
        test_loss, test_acc = evaluate(best_model, test_loader, criterion1, device)
