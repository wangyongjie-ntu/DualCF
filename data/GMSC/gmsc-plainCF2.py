...
if __name__ == "__main__":
    ...
    name = str(sys.argv[2])
    load_name = name + '/gmsc-plaincf-l2.npy'
    saved_name = name + '/gmsc-plaincf2-l2.npy'
    data1, data2 = load_GMSC("cs-training.csv")
    df = data1.iloc[0:int(len(data1) * 0.5)]
    dquery = np.load(load_name)
    dquery_x = dquery[:, 1:]
    dquery_y = dquery[:, 0]
    ...
    dquery_x = scaler.transform(dquery_x)

    clf = NNModel(model_path = 'gmsc_model.pt')
    cf = PlainCF(clf)

    gmsc_cf = np.zeros_like(df)

    # determine the initialization
    y_pred = clf.predict_ndarray(train)[0]
    idx0 = np.where(y_pred == 0)
    idx1 = np.where(y_pred == 1)
    init0 = train[idx1].mean(0)
    init1 = train[idx0].mean(0)
